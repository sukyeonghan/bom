...
public class AjaxPageBarFactory {
	...
	public static String getAjaxPageBar(int totalData,int cPage,int numPerpage,String url, String pdtNo) {
		...
		if(pageNo==1) {
			...
			pageBar+="<a class='w3-button' href=\"javascript:fn_paging("+(pageNo-1)+",\'"+pdtNo+"\')\"> << </a>";
		}
		...
		while(!(pageNo>pageEnd||pageNo>totalPage)) {
			...
			if(pageNo==cPage) { 
				...
				pageBar+="<a class='w3-button' href=\"javascript:fn_paging("+pageNo+",\'"+pdtNo+"\')\">"+pageNo+"</a>";
			}
			pageNo++;
		}
		...
		if(pageNo>totalPage) {
			...
			pageBar+="<a class='w3-button' href=\"javascript:fn_paging("+pageNo+",\'"+pdtNo+"\')\"> >> </a>";
		}
