	...
	<div class="media">
		...
		<div class="product-nav mr-3" style="min-width: 210px;">
			<ul>
            	...
            	<li><a class="select" href="${path }/product/pet">반려동물</a></li>
				                    	...
				                    	<c:choose>
				                    		<c:when test="${fn:contains(p.thumbs,',') }">
				                    			...
				                    			<c:forEach begin="0" end="0" var="th" items="${thumbs }" varStatus="v">
							                    	<img alt="" src="${path}/resources/upload/product/<c:if test='${v.count==1 }'>${th }</c:if>" 
							                    	class="item-img hover" style="display: inline;">
...
<script>
	...
	$(".sort").on("change",e=>{
		...
		$.ajax({
			...
			data:{"category":"욕실","sort":$(e.target).val()},
			type:"get",
			dataType:"html",
			success:data=>{
				console.log(data);
